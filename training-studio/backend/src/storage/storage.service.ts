...
// Mismo patrón que backend/src/storage/storage.service.ts de la plataforma
// principal: llama directo a la REST API de Supabase Storage (no al SDK) y
// puede apuntar a un proyecto/bucket Supabase completamente distinto — es
// solo configuración de un servicio externo de archivos, no un acoplamiento
// en tiempo de ejecución con el backend de la plataforma.
@Injectable()
export class StorageService {
  ...
  constructor(private config: ConfigService) {
    ...
    this.bucket = this.config.get<string>('SUPABASE_STORAGE_BUCKET') || 'training-documents';
  }
